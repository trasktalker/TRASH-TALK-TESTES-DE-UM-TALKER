...
const output = path.resolve(__dirname, '../artifacts/redesign');
const pages = {
...
(async () => {
  ...
  try {
    ...
    await context.route('**/api/**', async route => {
      ...
      else if (pathname === '/api/chats' && request.method() === 'POST') result = { id: 'preview' };
      ...
      else if (pathname === '/api/account/export') result = { user, chats: [chat()] };
    ...
    if (process.env.REDESIGN_BASELINE) {
      for (const file of Object.values(pages)) {
        const before = fs.readFileSync(path.join(process.env.REDESIGN_BASELINE, file), 'utf8');
        const after = fs.readFileSync(path.join(root, file), 'utf8');
        const diff = await page.evaluate(({ before, after }) => {
          function parse(source) {
            const doc = new DOMParser().parseFromString(source, 'text/html');
            doc.querySelectorAll('script, style, .skip-link').forEach(el => el.remove());
        ...
        assert.equal(diff.after.text, diff.before.text, file + ': original text changed');
    ...
    for (const theme of ['light', 'dark']) {
      ...
      for (const width of [320, 390, 768, 1024, 1440]) {
        ...
        for (const [url, file] of Object.entries(pages)) {
          ...
          if (['login.html', 'signup.html'].includes(file) && width >= 1024) {
            const art = await page.locator('.auth-page').evaluate(el => {
              const style = getComputedStyle(el, '::before');
              return { image: style.backgroundImage, background: style.backgroundColor };
            });
            assert(art.image.includes(theme === 'light' ? '/conversation-art-light.svg' : '/conversation-art.svg'), 'Auth illustration must follow the saved theme');
            assert.equal(art.background, theme === 'light' ? 'rgb(237, 236, 227)' : 'rgb(36, 29, 26)');
            ...
            }), theme === 'light' ? '/conversation-art-light.svg' : '/conversation-art.svg');
            assert(loaded, 'CSS illustration asset must load');
          }
          if (file === 'chat.html') {
          ...
          if (['index.html', 'dashboard.html', 'login.html', 'chat.html', 'signup.html', 'account.html'].includes(file) || (file === 'settings.html' && width === 1440)) {
    ...
    // Illustration responds immediately to the same theme setter used by settings.
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.goto(base + '/login');
    for (const theme of ['light', 'dark']) {
      await page.evaluate(theme => setTheme(theme), theme);
      assert((await page.locator('.auth-page').evaluate(el => getComputedStyle(el, '::before').backgroundImage))
        .includes(theme === 'light' ? '/conversation-art-light.svg' : '/conversation-art.svg'));
    ...
    for (const width of [320, 900]) {
      ...
      await page.locator('.personality-picker-trigger').waitFor();
    ...
    const picker = page.locator('.personality-picker-trigger');
    ...
    await page.waitForFunction(() => document.querySelector('.personality-picker-trigger').textContent.includes('Talker'));
    ...
    console.log(JSON.stringify({ passed: true, viewChecks, passwordChecks, contrastResults, originalCopyPreserved: !!process.env.REDESIGN_BASELINE, mockedAPIs: true, output }, null, 2));
  } finally {
